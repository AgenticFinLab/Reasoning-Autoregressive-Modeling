"""EQD Scales (Encoder-Quantizer-Decoder with Multi-Scale Quantization).

Unified model combining encoder, multi-scale quantizer (VAR-style), and decoder.
"""
...
class MultiScaleQuantizer(nn.Module):
    """Multi-Scale Quantizer (VAR-style).

    Uses configurable scale lengths for multi-scale residual quantization.
    """
    ...
    def __init__(
        ...
        scale_lengths: tuple = (32, 64, 128, 256, 512),
        beta: float = 0.25,
    ):
        super().__init__()
    ...
    def forward(self, z: torch.Tensor):
        ...
        for scale_len in self.scale_lengths:
            ...
            h_up = q.repeat_interleave(pool_size, dim=1)[:, :L, :]
            f_hat = f_hat + h_up
            f_rest = f_rest - h_up
...
class EQDScalesModel(nn.Module):
    """Unified EQD Scales Model."""
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.quantizer = MultiScaleQuantizer(
            ...
            scale_lengths=tuple(
                quant_cfg.get("scale_lengths", [32, 64, 128, 256, 512])
            ),
