...
def train_eqd_scales(config: dict, ds_config: dict, tee_logger: TeeLogger | None = None):
    ...
    if is_main_process:
        logger = TrainingLogger(
            name="eqd_scales_train",
            log_file=log_dir / "training.log",
        )
        logger.log_header("EQD Scales - DeepSpeed Training")
    else:
        logger = None
    ...
    eqd_model = EQDScalesModel(model_cfg)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="EQD Scales - DeepSpeed Training"
    )
    parser.add_argument(
        "--config",
        type=str,
