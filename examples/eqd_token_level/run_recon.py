"""EQD Token Level Reconstruction Evaluation Script.

Usage:
    python examples/eqd_token_level/run_recon.py -c configs/eqd_token_level/config.yaml
    python examples/eqd_token_level/run_recon.py -c configs/eqd_token_level/config.yaml -n 50

This script loads a trained EQD Token Level model from checkpoints and generates
...
def run_reconstruction(
    ...
    Args:
        model: Trained EQD Token Level model
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="EQD Token Level Reconstruction Evaluation",
    )
    parser.add_argument(
        "-c",
        "--config",
    ...
    for ckpt_path in ckpt_files:
        ...
        model = load_trained_model_for_recon(
            ...
            model_class=EQDTokenLevelModel,
            model_cfg=model_cfg,
            device=str(device),
        )
