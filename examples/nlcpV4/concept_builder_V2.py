"""NLCP V4 Concept Pyramid Builder — V2 (padding-geometry fix).

RELATIONSHIP TO V1
==================
``concept_builder.py`` (V1) is frozen because many experiments are
already tied to its exact forward.  V2 preserves the public surface
(``ConceptPyramidBuilder`` class, ``forward`` contract, dataclasses) and
changes only ONE thing: ``_prepare_reasoning`` no longer concatenates
``[Q_pad | C | S_pad]`` and then slices at fixed offsets.  Instead it
delegates to :func:`nlcpV4.utils.pack_qcs_sequences`, which packs
``[real_Q_i | C | real_S_i]`` per row with NO internal padding and
exposes per-row gather indices for the solution logits.

This fixes a padding-geometry bug: under right-padded Q with variable
lengths, the legacy slice ``logits[:, L_Q + total_C - 1 + j, :]`` lands
on positions whose RoPE distance to the last real Q token varies across
the batch, and the last-real-Q hidden for short rows was being computed
from pad-token context.  See the docstring on :meth:`_prepare_reasoning`
below for the packed forward contract.
...
class ConceptPyramidBuilder(nn.Module):
    ...
    def _prepare_reasoning(
        ...
        PRINCIPLE:
            ...
            The original reasoning flow is Q -> CoT -> Solution.  With
            concepts replacing CoT, the flow becomes Q -> Concepts -> S.

            V2 DIFFERENCE FROM V1 (padding-geometry fix):
                V1 concatenated ``[Q_pad | C | S_pad]`` and sliced
                solution logits at the batch-uniform offset
                ``L_Q + total_C - 1 + j``.  Under right-padded Q with
                variable lengths, that offset lands on positions whose
                RoPE distance to the last real Q token varies across the
                batch; additionally, for short rows the last-real-Q
                hidden is computed from PAD-token context.

                V2 delegates to :func:`nlcpV4.utils.pack_qcs_sequences`
                which packs ``[real_Q_i | C | real_S_i]`` per row with
                NO internal padding, then gathers solution logits via
                per-row advanced indices.  Every row's concept block
                starts exactly one position after its last real Q token,
            ...
            Data flow (teacher-forcing):
                ...
                3. embed Q / S tokens via the shared input embedding.
                4. pack_qcs_sequences -> PackedQCS (per-row layout,
                   no internal pads).
                5. Forward through reason_model on packed_embeds.
                6. gather_solution_logits via pack.solution_(row|col)_idx
                   -> [B, L_S, V]
                7. build_solution_targets: solution_ids with pad -> -100
                8. Store in pyramid + argmax decode for reasoning_texts.

        PURPOSE:
            Validate that the concept pyramid supports reasoning. A
            pyramid that perfectly reconstructs CoT hidden states but
            cannot produce the correct solution is useless.

        Args:
            pyramid: PyramidOutput from forward() — will be mutated in-place.
            question_ids: Token IDs for the question [B, L_Q_pad].
            question_attention_mask: Attention mask for question [B, L_Q_pad].
            solution_ids: Token IDs for the solution (target) [B, L_S_pad],
                right-padded.
            solution_attention_mask: Attention mask for solution [B, L_S_pad].
        """
        ...
        # Q_embeds: [B, L_Q_pad, D_enc];  S_embeds: [B, L_S_pad, D_enc]
        Q_embeds = embed_layer(question_ids)
        S_embeds = embed_layer(solution_ids)

        # Keep dtypes consistent with Q/S (back_decode may return float32).
        if concept_embeds.dtype != Q_embeds.dtype:
            concept_embeds = concept_embeds.to(Q_embeds.dtype)

        # Step 4: Pack per-row [real_Q_i | C | real_S_i] with NO internal pads.
        ...
        # logits: [B, T, V] where T = max(q_len + total_C + s_len)
        logits = outputs.logits

        # Step 6: Gather solution-prediction logits per row.
        # solution_logits: [B, L_S_pad, V] — padded positions are clamped
        # to T-1 during packing; CE ignores them via -100 targets below.
        solution_logits = gather_solution_logits(logits, pack)

        # Step 7: Build targets with -100 for padding positions.
        targets = build_solution_targets(solution_ids, solution_attention_mask, pack)

        # Step 8: Store in pyramid + teacher-forced argmax decode.
