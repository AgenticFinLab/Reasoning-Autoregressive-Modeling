...
def run_pipeline(config, device):
    ...
    # ==================================================================
    log_section("Step 2: Load GSM8K Data via NLCPV3DataLoader")
    ...
    # Reasoning loss (NTP) with real Q + solution
    ...
    total_all = total_loss.item() + reasoning_raw * reasoning_w
    logging.info(
        "  │ TOTAL                  │             │          │ %13.4f   │", total_all
    )
    logging.info(
        "  └─────────────────────────────────────────────────────────────────────┘"
    )
    ...
    log_check("reasoning loss is finite", torch.isfinite(reasoning_loss).item())
    log_check("reasoning loss is positive", reasoning_raw > 0)

    # ==================================================================
    # Step 6: Gradient Flow — backward through combined loss
    # ==================================================================
    log_section("Step 6: Gradient Flow (backward on total_loss + reasoning_loss)")

    builder.zero_grad()
    combined_loss = total_loss + reasoning_w * reasoning_loss
